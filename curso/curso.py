...
class course(osv.osv):
    ...
    def compare(self, cr,uid,ids,ds,df,error=0,context=None):
        if ds > df:
            if error == '1':
                raise osv.except_osv(_('ERROR!'), _('La fecha de inicio de curso no puede ser menor a la de inscripción'))
            elif error == 'm':
    ...
    def carga_unidades(self,cr,uid,ids,context=None):
        ...
        for unit in course_obj.learning_unit_id:
            ...
            for unit in unit_obj:
                self.pool.get('course.docentes').create(cr,uid,{'name':unit.name,'course_id':ids[0],'learning_unit_id' : unit.id})
        return True
    ...
    def _is_open(self, cr, uid, ids, fields, arg, context=None):
        res = dict()
        for course in self.browse(cr, uid, ids, context=context):
            if (datetime.datetime.strptime(course.start_date_inscription, '%Y-%m-%d') <= datetime.datetime.now()) and (datetime.datetime.strptime(course.finish_date_inscription, '%Y-%m-%d') >= datetime.datetime.now()):
                res[course.id] = True
            else:
                res[course.id] = False
        return res
    ...
    _columns = {
        ...
        "course_docentes_ids" : fields.one2many('course.docentes','course_id', 'Unidades de Aprendisajes', delete='cascade',),
